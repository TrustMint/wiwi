...
export const ModalProvider: React.FC<ModalProviderProps> = ({ children }) => {
  ...
  const handleTouchStart = useCallback((e: React.TouchEvent) => {
    ...
    if (!scrollableContent || scrollableContent.scrollTop > 0) return; // Разрешаем свайп только с самого верха
    ...
    const handleTouchMove = (moveEvent: TouchEvent) => {
      ...
      if (deltaY > 0 && content) {
        ...
        const resistance = 0.5; // "Резиновый" эффект
    ...
    const handleTouchEnd = (endEvent: TouchEvent) => {
      ...
      if(content) {
        ...
        if (deltaY > 100) { // Порог для закрытия свайпом
          hideModal();
        } else {
        ...
        >
          <div
            ...
            className={`
              ...
              rounded-[32px] /* Слегка уменьшил скругление для современного вида */
              shadow-2xl
              transition-all duration-300 ease-in-out
              flex flex-col
