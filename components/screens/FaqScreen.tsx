...
const FaqCategoryRow: React.FC<{ title: string; icon: React.ReactNode; onClick: () => void }> = ({ title, icon, onClick }) => (
    <div onClick={onClick} className="flex items-center p-3 cursor-pointer hover:bg-white/5 transition-colors">
        {icon}
        <span className="flex-1 text-white text-base ml-4">{title}</span>
                ...
                <GlassPanel className="overflow-hidden divide-y divide-white/10">
                    {faqData.map((category) => (
                        <FaqCategoryRow 
                            ...
                            icon={<IOSIcon emoji={category.emoji} />}
