...
import { ChevronLeftIcon, ShareIcon, InformationCircleIcon, SparklesIcon, PlayCircleIcon, PencilSquareIcon, ReputationMedalIcon, ClipboardDocumentListIcon } from '../icons/Icons';
...
export const ItemScreen: React.FC<ItemScreenProps> = ({
  ...
  // Identify available variant keys (e.g. "Color", "Size")
  ...
  useEffect(() => {
    const observer = new ResizeObserver(entries => {
      for (const entry of entries) {
        ...
        // Check if panel is actually visible (height > 0) before setting padding
        if (panel.offsetHeight > 0 && window.getComputedStyle(panel).display !== 'none') {
            const panelHeight = panel.offsetHeight;
            // Increased bottom padding to ensure scroll content isn't hidden behind buttons
  ...
  // FIX: Converted to a render function to avoid component remounts causing event loss
  const renderFloatingActions = (isMobile: boolean) => {
      // Common button styles to ensure consistent look
      ...
      if (isMyListing) {
          return (
            <div className={`flex flex-col gap-3 w-full ${isMobile ? 'pointer-events-none' : ''}`}>
                ...
                {listing.status === 'Sold' && (
                    <div className={`p-3 w-full text-center text-green-400 font-bold bg-green-500/10 rounded-2xl border border-green-500/20 backdrop-blur-md ${isMobile ? 'pointer-events-auto' : ''}`}>
      ...
      return (
        <div className={`flex flex-col gap-3 w-full ${isMobile ? 'pointer-events-none' : ''}`}>
             ...
             {listing.status === 'In Escrow' && user?.address === listing.buyer?.address && (
                 <div className={`flex flex-col gap-3 w-full ${isMobile ? 'pointer-events-none' : ''}`}>
                     <button 
