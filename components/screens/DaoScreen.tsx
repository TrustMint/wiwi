...
import { mockProposals } from '../../services/mockData';
    ...
    const VoterRow: React.FC<{vote: Vote}> = ({ vote }) => (
        <div className="flex items-center justify-between py-2">
            <div className="flex items-center gap-3">
                <img src={vote.voterAvatar} className="w-8 h-8 rounded-full" />
                <div>
                    ...
                    <div className="flex justify-between items-center">
                         <div className="flex space-x-1 p-1 bg-black/20 rounded-lg">
                            <button 
                                ...
                                className={`px-4 py-1.5 rounded-md text-sm font-medium transition-colors ${activeTab === 'active' ? 'bg-white/10 backdrop-blur-sm ring-1 ring-white/15 text-white shadow-md' : 'text-gray-300 hover:bg-white/5'}`}
                            >
                                Активные
                            </button>
                            <button 
                                onClick={() => setActiveTab('completed')}
                                className={`px-4 py-1.5 rounded-md text-sm font-medium transition-colors ${activeTab === 'completed' ? 'bg-white/10 backdrop-blur-sm ring-1 ring-white/15 text-white shadow-md' : 'text-gray-300 hover:bg-white/5'}`}
                            >
                                Завершенные
                            </button>
                        </div>
