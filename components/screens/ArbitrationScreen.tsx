...
import { mockArbitrationCases } from '../../services/mockData';
            ...
            <main className="flex-1 overflow-y-auto p-4 space-y-6 pb-28">
                <div className="flex space-x-1 p-1 bg-black/20 rounded-lg">
                    <button 
                        ...
                        className={`w-full py-1.5 rounded-md text-sm font-medium transition-colors ${activeTab === 'voting' ? 'bg-white/10 backdrop-blur-sm ring-1 ring-white/15 text-white shadow-md' : 'text-gray-300 hover:bg-white/5'}`}
                    >
                        Требуют внимания
                    </button>
                    <button 
                        onClick={() => setActiveTab('resolved')}
                        className={`w-full py-1.5 rounded-md text-sm font-medium transition-colors ${activeTab === 'resolved' ? 'bg-white/10 backdrop-blur-sm ring-1 ring-white/15 text-white shadow-md' : 'text-gray-300 hover:bg-white/5'}`}
                    >
                        Закрытые дела
                    </button>
                </div>
                ...
                <div className="space-y-4">
                     {filteredCases.length > 0 ? (
                        ...
                        <div className="text-center text-gray-400 py-10">
                            ...
                            <p className="mt-2 font-semibold">Нет дел</p>
                            <p className="text-sm">В этой категории пока нет дел.</p>
                        </div>
                    )}
                </div>
            </main>
...
const ArbitrationHub: React.FC<{ onBack: () => void; onNavigate: (screen: 'case_list') => void; }> = ({ onBack, onNavigate }) => {
    ...
    const onTouchEndNavigate = getTouchEndHandler(handleNavigate);
            ...
            <main className="flex-1 overflow-y-auto p-4 space-y-6 pb-28">
                ...
                <GlassPanel>
                    <div 
                        ...
                        onTouchStart={canViewCases ? onTouchStart : undefined}
                        onTouchEnd={canViewCases ? onTouchEndNavigate : undefined}
...
export const ArbitrationScreen: React.FC<ArbitrationScreenProps> = ({ onBack }) => {
    const [cases] = useState<ArbitrationCase[]>(mockArbitrationCases);
