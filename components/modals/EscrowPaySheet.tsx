...
export const EscrowPaySheet: React.FC<EscrowPaySheetProps> = ({ listing, onConfirm }) => {
    const { hideModal } = useModal();
    const [isProcessing, setIsProcessing] = useState(false);
    const [purchasedQuantity, setPurchasedQuantity] = useState(1);

    // Recalculate prices based on selected quantity
    const itemPrice = listing.price * purchasedQuantity;
    ...
    const handleQuantityChange = (delta: number) => {
        ...
        if (newQuantity >= 1 && newQuantity <= listing.quantity) {
        ...
        <div className="p-4 space-y-2 text-center">
            ...
            <div className="py-2 space-y-1.5 text-left border-y border-white/10">
                <div className="flex justify-between items-start gap-4">
                    ...
                    <span className="text-white font-medium text-right truncate text-xs">{listing.title}</span>
                </div>
                 <div className="flex justify-between items-start gap-4">
                 ...
                 <div className="flex justify-between items-center">
                    ...
                    <div className="flex items-center space-x-1 bg-white/5 rounded-md p-0.5">
                        ...
                        <button onClick={() => handleQuantityChange(1)} disabled={purchasedQuantity >= listing.quantity} className="px-1.5 rounded text-base font-bold disabled:opacity-50">+</button>
                    </div>
                </div>
            </div>
            ...
            <button 
                ...
                disabled={isProcessing || listing.quantity === 0}
