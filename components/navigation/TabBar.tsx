...
const TabBarItemRenderer: React.FC<TabBarItemRendererProps> = ({
  ...
  if (isAction) {
    return (
      ...
      >
        <div className={`
          flex items-center justify-center w-14 h-12 rounded-2xl text-white shadow-lg transform transition-all duration-200
          ${disabled 
        ...
        {badge && badge > 0 && (
          <span className="absolute -top-1 -right-1 bg-red-500 text-white text-xs font-bold rounded-full min-w-[18px] h-[18px] flex items-center justify-center px-1 border-2 border-black">
            {badge > 99 ? '99+' : badge}
          </span>
        )}
      </button>
  ...
  return (
    ...
    >
      ...
      <span 
        className={`
          absolute inset-1 backdrop-blur-md ring-1 rounded-full transition-all duration-300 ease-in-out 
          ${isActive 
            ? 'bg-white/5 ring-white/20 opacity-100 scale-100' 
            : 'opacity-0 scale-90'
          }
          ${disabled ? 'ring-gray-600' : 'ring-white/20'}
        `}
        aria-hidden="true"
      ></span>
      ...
      <div className="relative flex flex-col items-center justify-center space-y-1">
        ...
        `}>
          {icon}
          {badge && badge > 0 && (
            <span className="absolute -top-2 -right-2 bg-red-500 text-white text-xs font-bold rounded-full min-w-[18px] h-[18px] flex items-center justify-center px-1 border-2 border-black">
              {badge > 99 ? '99+' : badge}
            </span>
          )}
        </div>
...
const TabBarComponent: React.FC<TabBarProps> = ({ children, activeTab, onTabChange }) => {
  ...
  return (
    <div 
      className="fixed inset-x-4 bottom-4 z-50 pb-[env(safe-area-inset-bottom)] pointer-events-none"
      role="navigation"
      aria-label="Основная навигация"
    >
        <div 
          className="max-w-md mx-auto bg-black/50 backdrop-blur-xl rounded-full ring-1 ring-white/20 shadow-lg pointer-events-auto"
          role="tablist"
